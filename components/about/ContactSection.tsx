...
import { BTN_HOVER, BTN_TAP, SectionTransition, SPRING_HOVER } from "@/components/motion";

const rowClass =
  "flex items-center gap-4 group w-full text-left sm:justify-center";
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center text-center [perspective:1000px]">
        ...
        <p className="text-muted text-sm max-w-md leading-relaxed mb-12">
          Whether you&apos;re an individual, SME, or corporate organization — we are ready to
          help you find the right insurance coverage. Reach out today.
        </p>

        <div className="flex flex-col gap-5 max-w-md w-full">
          <ContactRow
            href="mailto:zahabusolutions@gmail.com"
            label="zahabusolutions@gmail.com"
            reduce={reduce}
            icon={<EmailIcon />}
          />
          <ContactRow
            href="tel:+250788381630"
            label="+250 788 381 630"
            reduce={reduce}
            icon={<PhoneIcon />}
          />
          <motion.div
            className={rowClass + " sm:pl-0 cursor-default"}
            whileHover={reduce ? undefined : { y: -2, x: 2 }}
            transition={SPRING_HOVER}
          >
            <div className="w-12 h-12 rounded-full border-2 border-primary/20 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
              <MapIcon />
            </div>
            <span className="text-primary font-bold text-lg tracking-wide">Kigali, Rwanda</span>
          </motion.div>
        </div>
      </div>
    </SectionTransition>
...
function ContactRow({
  ...
  if (reduce) {
    return (
      <a href={href} className={rowClass + " group"}>
  ...
  return (
    <motion.a
      href={href}
      className={rowClass + " group rounded-xl p-1 -m-1"}
      whileHover={{
        ...BTN_HOVER,
        x: 2,
    ...
    >
      <div className="w-12 h-12 rounded-full border-2 border-primary/20 flex items-center justify-center group-hover:bg-primary group-hover:border-primary transition-all duration-200">
        <span className="text-primary group-hover:text-white transition-colors [&_svg]:text-inherit">
          {icon}
        </span>
      </div>
      <span className="text-primary font-bold text-lg tracking-wide uppercase group-hover:text-accent transition-colors">
        {label}
      </span>
    </motion.a>
  );
